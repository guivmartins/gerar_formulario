...
if st.session_state.formulario["secoes"]:
    ...
    with st.expander(f"➕ Adicionar Campos à seção: {secao_atual['titulo']}", expanded=True):
        ...
        if st.button("Adicionar Campo"):
            ...
            # Se for grupoRadio ou grupoCheck, criar domínio global separado
            if tipo in ["grupoRadio", "grupoCheck"] and dominios:
                chave = titulo.replace(" ", "")[:20].upper()
                contador = 1
                chave_final = chave
                while chave_final in st.session_state.formulario["dominios"]:
                    chave_final = f"{chave}{contador}"
                    contador += 1
                st.session_state.formulario["dominios"][chave_final] = dominios
                campo["dominio"] = chave_final
